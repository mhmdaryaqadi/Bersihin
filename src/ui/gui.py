...
class App(ctk.CTk):
    ...
    def initialize_main_app(self, start_minimized=False):
        ...
        self.create_nav_button("Pengaturan", SettingsFrame, 4)
        
        # Admin Privilege Badge
        self.priv_badge = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        self.priv_badge.grid(row=6, column=0, padx=15, pady=(10, 5), sticky="ew")
        ...
        if not is_adm:
            ...
            self.btn_relaunch.grid(row=7, column=0, padx=20, pady=(5, 20), sticky="ew")
        else:
            # Placeholder padding
            self.pad_lbl = ctk.CTkLabel(self.sidebar, text="", height=20)
            self.pad_lbl.grid(row=7, column=0, pady=(5, 10))
        ...
        self.frames = {}
        for F in (DashboardFrame, RAMCleanerFrame, FileCleanerFrame, SettingsFrame):
